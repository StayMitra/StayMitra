...
/// that a real OCR/API service can be connected later without changing the
/// screen flow.
...
class _UploadRegisterScreenState extends State<UploadRegisterScreen> {
  ...
  Future<void> _takePhoto() async {
    try {
      ...
      _showMessage('Unable to capture the register photo.');
    }
  }
  ...
  Future<void> _pickFromGallery() async {
    try {
      ...
      _showMessage('Unable to select register photos.');
    }
  }
  ...
  Future<void> _editTenant(int index) async {
    ...
    final bedController = TextEditingController(text: tenant.bed);
  ...
  Future<void> _confirmImport() async {
    if (_tenants.isEmpty) {
      _showMessage('Please upload a register first.');
      return;
    }

    final validTenants = _tenants
        .where((tenant) => tenant.name.trim().isNotEmpty)
        .toList();
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Import Tenants?'),
          content: Text(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  hasName ? tenant.name : 'Tenant details needed',
                  maxLines: 1,
  ...
  String _tenantSubtitle(_ExtractedTenant tenant) {
    final parts = <String>[];

    if (tenant.room.isNotEmpty) {
      parts.add('Room ${tenant.room}');
    }

    if (tenant.bed.isNotEmpty) {
      parts.add('Bed ${tenant.bed}');
    }

    if (tenant.rent.isNotEmpty) {
      parts.add('Rent ₹${tenant.rent}');
    }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _imageSection(),
              _processingCard(),
              if (!_isProcessing && _registerImages.isNotEmpty) ...[
                const SizedBox(height: 16),
                _tenantSection(),
                const SizedBox(height: 16),
                _howItWorksCard(),
