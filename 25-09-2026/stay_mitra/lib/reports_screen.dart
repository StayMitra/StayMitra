...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Future<void> _loadReports() async {
    try {
      ...
      // ----------------------------------------------------------
      // IMPORTANT:
      // Reports are strictly for the currently selected PG.
      // PgManagerService contains the currently selected property.
      // ----------------------------------------------------------
      ...
      final floors =
          await FirebaseTenantService.instance.getFloors(
      ...
      // ----------------------------------------------------------
      // LOAD ROOMS AND BEDS FROM FIREBASE
      // ----------------------------------------------------------

      for (final floor in floors) {
        final rooms =
            await FirebaseTenantService.instance.getRooms(
          propertyId: buildingId,
          floorId: floor.id,
        );
        ...
        for (final room in rooms) {
          final bedSnapshot = await _firestore
              .collection('owners')
              .doc(ownerId)
          ...
          for (final bedDoc in bedSnapshot.docs) {
            final data = bedDoc.data();
      ...
      // ----------------------------------------------------------
      // LOAD ACTIVE TENANTS FROM FIREBASE
      // ----------------------------------------------------------
