...
class _DashboardPageState extends State<DashboardPage> {
  ...
  String _t(String key) => LanguageService.instance.get(key);

  String _dashboardLabel(String text) {
    const keys = <String, String>{
      'Dashboard': 'dashboard',
      'Tenants': 'tenants',
      'Rooms': 'rooms',
      'Rooms & Beds': 'rooms_beds',
      'Day Sheet': 'day_sheet',
      'Billing': 'billing',
      'Accounts': 'accounts',
      'Reports': 'reports',
      ...
      'Occupied': 'occupied',
      'Vacant': 'vacant',
      'Today Collection': 'today_collection',
      'Payments': 'payments',
      ...
      'PG/Apartment Structure': 'pg_apartment_structure',
      'Welcome': 'welcome',
      'Current property': 'current_property',
      'No PG configured': 'no_pg_configured',
      'Set up your PG to get started': 'setup_pg_to_get_started',
      'Menu': 'menu',
    };
    final key = keys[text];
    return key == null ? text : _t(key);
  }
        ...
        children: [
          ...
          Text(
            title == 'Today Collection'
                ? '$todayPaymentCount ${_t('payments')}'
                : title == 'Pending Rent'
                    ? (_propertyStatsLoading
                        ? _t('loading')
                        : '$_pendingRentTenants ${_t('active_tenants')}')
                    : _dashboardLabel(subtitle),
            maxLines: 1,
            overflow:
                TextOverflow.ellipsis,
            style: const TextStyle(
...
builder: (context) =>
    create_bill_screen.CreateBillScreen(
      ...
      buildingName: _building?.name,
          ...
          // CHECK-IN
          if (title == 'Check-in/Out' || title == 'Check-in') {
            final result = await Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) =>
          ...
          ScaffoldMessenger.of(context)
            ...
            ..showSnackBar(
              SnackBar(
                content:
                    Text('${_dashboardLabel(title)} selected'),
                behavior:
                    SnackBarBehavior.floating,
              ),
            );
