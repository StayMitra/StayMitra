...
class _CreateBillScreenState extends State<CreateBillScreen> {
  ...
  Future<void> _loadTenants() async {
    ...
    try {
      ...
      // FIREBASE ONLY
      // ========================================================
      // If a PG / Building is provided, load tenants only from
      // that property's Firebase tenants collection.
      //
      // If no PG / Building is provided, load active tenants from
      // all properties belonging to the logged-in Firebase user.
      // ========================================================

      if (selectedBuildingId != null &&
          selectedBuildingId.isNotEmpty) {
        final propertyTenants =
            await FirebaseTenantService.instance.getTenants(
        ...
        tenants = propertyTenants
            .where(
              (tenant) =>
                  tenant.status.trim().toLowerCase() ==
                  'active',
            )
            .toList();
      } else {
      ...
      debugPrint(
        'CREATE BILL - FIREBASE TENANT LOAD ERROR: $e',
      );
...
Future<void> _generateBill() async {
    ...
    try {
      ...
      debugPrint(
        'Building ID from widget: ${widget.buildingId}',
      );
      ...
      debugPrint(
        'Generated Bill ID: $billId',
      );

      // ============================================================
      // RESOLVE BUILDING ID FROM FIREBASE
      // ============================================================
       ...
       * find the building through Firebase:
       *
       * owners/{ownerId}/properties/{propertyId}/tenants/{tenantId}
       *
       * The tenant is searched across the owner's properties.
       */

      if (resolvedBuildingId == null ||
          resolvedBuildingId.isEmpty) {
        debugPrint(
          'Building ID missing. Trying to find building from Firebase tenant...',
        );
      ...
      /*
       * Firebase bills are stored under the selected property.
       * A property ID is therefore required for the bill document.
       */
      if (resolvedBuildingId == null ||
          resolvedBuildingId.isEmpty) {
        debugPrint(
          'WARNING: Building ID could not be resolved from Firebase.',
        );

        throw Exception(
          'Building ID could not be resolved for the selected tenant.',
        );
      }
      ...
      // ============================================================
      // INSERT BILL INTO FIREBASE FIRESTORE
      // ============================================================

      debugPrint(
        '========== INSERTING BILL INTO FIREBASE =========='
      );
      ...
      debugPrint(
        'FIREBASE BILL DOCUMENT SAVED: ${billRef.path}',
      );
