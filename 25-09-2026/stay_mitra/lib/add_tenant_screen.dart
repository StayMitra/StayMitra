...
class _AddTenantScreenState extends State<AddTenantScreen> {
  ...
  Future<void> _loadBuildings() async {
    try {
      ...
      // ==========================================================
      //
      // The currently selected PG is used as the propertyId.
      // This keeps tenants, floors, rooms and beds isolated per PG.
      // The same code path works from the Firebase Flutter plugins
      // on Android, iOS, web, macOS and the supported desktop targets.
      // ==========================================================
      ...
      final buildings =
          await FirebaseTenantService.instance
              .getBuildings(
        propertyId: activePg.id,
      );
      ...
      _showError(
        'Unable to load buildings from Firebase.',
      );
    }
  }
  ...
  Future<void> _selectBuilding(
    ...
    try {
      ...
      _showError(
        'Unable to load floors from Firebase.',
      );
    }
  }
  ...
  Future<void> _selectRoom(
    ...
    try {
      ...
      _showError(
        'Unable to load available beds from Firebase.',
      );
    }
  }
  ...
  Future<void> _pickIdProofPhoto(
    ...
    try {
      // XFile is used instead of dart:io File so the same screen
      // can run on Android, iOS, web, macOS and supported desktop.
      ...
      // If already <= 150 KB, keep the original XFile.
      if (originalSize <= _maxPhotoSizeBytes) {
        ...
        setState(() {
          _idProofPhoto = pickedFile;
        });

        return;
      }

      // If larger than 150 KB, compress the image bytes.
  ...
  Future<XFile?> _compressPhotoTo150KB(
    XFile originalFile,
  ) async {
    try {
      final originalBytes =
  ...
  Future<void> _saveTenant() async {
    ...
    try {
      ...
      // Upload the selected ID proof first, then save the tenant document
      // with the resulting Firebase Storage URL.
      String? idProofUrl;

      if (_idProofPhoto != null) {
        final proofType = _selectedIdProofType?.trim();
        if (proofType == null || proofType.isEmpty) {
          throw Exception('Please select ID proof type.');
        }

        final photoBytes = await _idProofPhoto!.readAsBytes();
        if (photoBytes.isEmpty) {
          throw Exception('ID proof photo is empty.');
        }

        idProofUrl =
            await FirebaseTenantService.instance.uploadTenantIdProof(
          propertyId: activePg.id,
          tenantId: tenant.id,
          proofType: proofType,
          bytes: photoBytes,
  ...
  String message =
      'Unable to add tenant: $e';

  final error =
      e.toString().toLowerCase();

  if (error.contains('already occupied')) {
    message =
    ...
    message =
        'Firebase permission denied. Please check Firebase Storage/Firestore rules.';
  } else if (error.contains('unauthenticated')) {
    message =
        'Firebase login session expired. Please login again.';
  } else if (error.contains('network-request-failed')) {
    message =
        'Network error while uploading ID proof. Please check internet connection.';
  }
