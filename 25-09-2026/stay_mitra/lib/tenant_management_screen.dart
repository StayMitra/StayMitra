...
class _TenantManagementScreenState
  ...
  Future<void> _loadTenants() async {
    ...
    try {
      // Load tenants only for the currently selected PG.
      // Firestore path: owners/{uid}/properties/{propertyId}/tenants
      ...
      if (activePg == null) {
        ...
        _showError('No active PG is selected.');
        return;
      }
      ...
      _showError('Unable to load tenants from Firebase.');
    }
  }
  ...
  Future<void> _showIdProof(
    TenantModel tenant,
  ) async {
    final url = tenant.idProofUrl?.trim();

    if (url == null || url.isEmpty) {
      _showError('ID proof photo is not available for this tenant.');
      return;
    }
          ...
          child: SafeArea(
            child: Column(
              ...
              children: [
                Padding(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        child: Text(
                          '${tenant.idProofType ?? 'ID Proof'} - ${tenant.fullName}',
                          maxLines: 1,
                        ...
                        return const SizedBox(
                          ...
                          child: Center(
                            child: Padding(
                              ...
                              child: Text(
                                'Unable to load the ID proof photo.',
                ...
                if (tenant.idProofNumber?.trim().isNotEmpty == true)
                  Padding(
                    padding: const EdgeInsets.fromLTRB(16, 10, 16, 16),
                    child: Text(
                      'ID Number: ${tenant.idProofNumber!}',
                      textAlign: TextAlign.center,
                      style: const TextStyle(
  ...
  Future<void> _vacateTenant(
    ...
    try {
      ...
      await FirebaseTenantService.instance.vacateTenant(
        propertyId: activePg.id,
        tenantId: tenant.id,
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${tenant.fullName} vacated successfully. Bed is now available.'),
          behavior: SnackBarBehavior.floating,
        ),
      );
    } catch (e) {
      if (!mounted) return;
      _showError('Unable to vacate tenant.');
    }
  }
  ...
  Future<void> _deleteTenant(
    ...
    try {
      ...
      await FirebaseTenantService.instance.deleteTenant(
        propertyId: activePg.id,
        tenantId: tenant.id,
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${tenant.fullName} deleted successfully.'),
          behavior: SnackBarBehavior.floating,
        ),
      );
    } catch (e) {
      if (!mounted) return;
      _showError('Unable to delete tenant.');
    }
  }
