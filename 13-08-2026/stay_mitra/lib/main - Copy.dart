...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<void> _loadPropertyStats() async {
    try {
      ...
      // PENDING RENT
      // ----------------------------------------------------------
      // Pending rent must be:
      //
      //   Monthly rent - payments received from that tenant
      //
      // Payments are currently stored in the transactions table.
      // The current TransactionModel does not yet contain a
      // tenantId, so Add Payment stores the tenant name in the
      // transaction description. We use that existing value here.
      //
      // Only current-month income transactions are considered.
      // This makes a ₹6,000 payment for a ₹6,000 monthly rent
      // reduce that tenant's pending rent to ₹0.
      // ----------------------------------------------------------
      ...
      for (final tenant in activeTenants) {
        ...
        final tenantName =
            (tenant['full_name'] as String).trim().toLowerCase();

        final monthlyRent =
            (tenant['monthly_rent'] as num?)?.toDouble() ?? 0.0;
        ...
        for (final transaction in transactions) {
          ...
          // Current payment flow stores the tenant name in the
          // description. Keep tenantId reserved for future billing
          // support without breaking existing saved transactions.
          final matchesTenant =
              tenantName.isNotEmpty &&
              description.contains(tenantName);

          // Also support future descriptions that may contain the
          // tenant ID if we introduce tenant-linked transactions.
          final matchesTenantId =
              description.contains(tenantId.toLowerCase());

          if (matchesTenant || matchesTenantId) {
            paidThisMonth += transaction.amount;
          }
        }

        final pendingForTenant =
            (monthlyRent - paidThisMonth).clamp(0.0, double.infinity);

        totalPendingRent += pendingForTenant;
      ...
      int countFrom(List<Map<String, Object?>> rows) {
      ...
      setState(() {
        ...
        _occupiedBeds = countFrom(occupiedResult);
        _availableBeds = countFrom(availableResult);
        _activeTenants = countFrom(tenantResult);

        _pendingRent = totalPendingRent;
        _pendingRentTenants = pendingTenantCount;
    ...
    return Material(
      ...
      child: InkWell(
        ...
        onTap: () async {
          ...
          // ADD TENANT
          // --------------------------------------------------------
          // Quick Actions -> Add Tenant opens the existing
          // AddTenantScreen. When a tenant is saved successfully,
          // the dashboard statistics are refreshed.
          if (title == 'Add Tenant') {
          ...
          if (title == 'Add Payment') {
            ...
            if (result == true) {
              // Payment changes both the financial cards and
              // the tenant's pending rent. Reload the dashboard
              // statistics so Pending Rent becomes ₹0 immediately
              // when the full monthly rent is received.
              await _refreshDashboard();
            }
