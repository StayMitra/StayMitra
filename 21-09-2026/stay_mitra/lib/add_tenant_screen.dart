...
class _AddTenantScreenState extends State<AddTenantScreen> {
  ...
  Future<void> _loadBuildings() async {
    try {
      // When Add Tenant is opened from the dashboard, always keep the
      // tenant inside the currently selected PG. This prevents tenants
      // from one PG appearing under another PG.
      ...
      setState(() {
        ...
        _selectedBuilding =
            activePg != null ? activePg : null;
        _isLoading = false;
      });

      // Load floors immediately for the active PG.
      if (activePg != null) {
        await _selectBuilding(activePg);
      }
    } catch (e) {
      if (!mounted) {
        return;
