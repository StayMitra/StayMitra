...
class DatabaseHelper {
  ...
  Future<Database> _initDatabase() async {
    ...
    return openDatabase(
      ...
      // IMPORTANT:
      // Version 7 is required because the migration below
      // contains the building_id migration.
      version: 10,
  ...
  Future<void> _onCreate(
    ...
    await db.execute('''
      CREATE INDEX idx_transactions_building_id
...
await db.execute('''
  CREATE INDEX IF NOT EXISTS
  idx_bills_building_id
  ON bills(building_id)
''');
    ...
    await db.transaction(
      (txn) async {
        ...
        if (buildingResult.isEmpty) {
          throw Exception(
            'Unable to determine tenant building.',
          );
        }
...
Future<int> insertBill(BillModel bill) async {
  ...
  if (columns.contains('bill_type')) {
    data['bill_type'] ??= 'Monthly Rent';
  }

  if (columns.contains('tenant_name')) {
    final tenantRows = await db.query(
      'tenants',
      columns: ['full_name'],
      where: 'id = ?',
      whereArgs: [bill.tenantId],
      limit: 1,
    );

    if (tenantRows.isNotEmpty) {
      data['tenant_name'] ??=
          tenantRows.first['full_name']?.toString();
    }
  }

  final compatibleData = <String, dynamic>{};

  for (final entry in data.entries) {
    if (columns.contains(entry.key)) {
      compatibleData[entry.key] = entry.value;
    }
  }
