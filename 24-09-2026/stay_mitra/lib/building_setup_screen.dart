...
Future<void> _loadStructure() async {
  ...
  try {
    ...
    // Firebase is the single source of truth for PG structure.
    // Do not fall back to local SQLite data here. This prevents
    // deleted Firebase floors/rooms/beds from being recreated.
    final floors = await _getFloorsFromFirebase(activePg.id);
  ...
  String get _firebaseOwnerId {
    return FirebaseAuth.instance.currentUser?.uid ?? widget.ownerId;
  }

  DocumentReference<Map<String, dynamic>> _firebasePropertyRef(
    String propertyId,
  ) {
  ...
  Future<List<FloorModel>> _getFloorsFromFirebase(
    String propertyId,
  ) async {
    final snapshot = await _firebasePropertyRef(propertyId)
        .collection('floors')
        .get();
    ...
    final floors = snapshot.docs.map((doc) {
      ...
      return FloorModel(
        ...
        floorOrder: _toInt(
          data['floor_order'] ?? data['floorOrder'],
        ),
        createdAt: _firebaseDate(
    ...
    floors.sort(
      (a, b) => a.floorOrder.compareTo(b.floorOrder),
    );

    return floors;
  }

  Future<List<RoomModel>> _getRoomsFromFirebase(
    String propertyId,
    String floorId,
  ) async {
    try {
      ...
      debugPrint('Error loading rooms from Firebase: $e');
      return <RoomModel>[];
    }
  }

  Future<List<BedModel>> _getBedsFromFirebase(
    String propertyId,
    String roomId,
  ) async {
    try {
      ...
      final beds = snapshot.docs.map((doc) {
        final data = doc.data();
      ...
      beds.sort(
        (a, b) => a.bedNumber.compareTo(b.bedNumber),
      );

      return beds;
    } catch (e) {
      debugPrint('Error loading beds from Firebase: $e');
      return <BedModel>[];
    }
  }

  Future<void> _saveFloorToFirebase(
    String propertyId,
    FloorModel floor,
  ) async {
    await _firebasePropertyRef(propertyId)
        .collection('floors')
        .doc(floor.id)
        .set({
      ...
      'created_at': Timestamp.fromDate(floor.createdAt),
      'createdAt': Timestamp.fromDate(floor.createdAt),
    });
  }

  Future<void> _saveRoomToFirebase(
    String propertyId,
    RoomModel room,
  ) async {
    await _firebasePropertyRef(propertyId)
      ...
      'created_at': Timestamp.fromDate(room.createdAt),
      'createdAt': Timestamp.fromDate(room.createdAt),
    });
  }

  Future<void> _saveBedToFirebase(
    String propertyId,
    BedModel bed,
  ) async {
    await _firebasePropertyRef(propertyId)
      ...
      'status': bed.status,
      'created_at': Timestamp.fromDate(bed.createdAt),
      'createdAt': Timestamp.fromDate(bed.createdAt),
    });
  }

  Future<void> _updateFloorInFirebase(
    String propertyId,
    FloorModel floor,
  ) async {
    await _firebasePropertyRef(propertyId)
  ...
  Future<void> _updateRoomInFirebase(
    String propertyId,
    RoomModel room,
  ) async {
    await _firebasePropertyRef(propertyId)
  ...
  Future<void> _deleteRoomFromFirebase(
    String propertyId,
    String roomId,
  ) async {
    final propertyRef = _firebasePropertyRef(propertyId);

    final bedsSnapshot = await propertyRef
        .collection('beds')
        .where(
          'room_id',
          isEqualTo: roomId,
    ...
    for (final bed in bedsSnapshot.docs) {
    ...
    batch.delete(
      propertyRef.collection('rooms').doc(roomId),
    );

    await batch.commit();
  }

  Future<void> _deleteFloorFromFirebase(
    String propertyId,
    String floorId,
  ) async {
    final propertyRef = _firebasePropertyRef(propertyId);

    final roomsSnapshot = await propertyRef
        .collection('rooms')
        .where(
          'floor_id',
          isEqualTo: floorId,
  ...
  Widget _floorCard(
    ...
    return FutureBuilder<List<RoomModel>>(
      future:
          _getRoomsFromFirebase(
        _building?.id ?? floor.buildingId,
        floor.id,
      ),
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                await _firebasePropertyRef(
                  building.id,
                ).set({
                  'name': building.name,
                  'address': building.address,
                  'ownerId': _firebaseOwnerId,
                  'createdAt': Timestamp.fromDate(
                    building.createdAt,
                  ),
                }, SetOptions(merge: true));

                if (PgManagerService.activePg == null) {
...
Future<void> _showAddFloorDialog() async {
  ...
  try {
    ...
    // Dialog completely closed first.
  ...
  Future<void> _deleteFloor(
    ...
    await _deleteFloorFromFirebase(
      floor.buildingId,
      floor.id,
    );
...
messenger.showSnackBar(
  ...
  Future<bool> _createRooms(
    ...
    // Firebase is the single source of truth. An empty Firebase
    // result means there are currently no rooms on this floor.
    final existingRooms = await _getRoomsFromFirebase(
      floor.buildingId,
      floor.id,
    );
  ...
  Future<void> _deleteRoom(
    ...
    final beds = await _getBedsFromFirebase(
      _building?.id ?? '',
      room.id,
    );
    ...
    // The Firebase read above is an async gap.
...
class _BedManagementDialogState extends State<_BedManagementDialog> {
  ...
  DocumentReference<Map<String, dynamic>> _propertyRef() {
    return FirebaseFirestore.instance
        .collection('owners')
        .doc(_ownerId)
        .collection('properties')
  ...
  Future<List<BedModel>> _loadBedsFromFirebase() async {
    try {
      final snapshot = await _propertyRef()
          .collection('beds')
          .where(
            'room_id',
            isEqualTo: widget.room.id,
          )
          .get();
      ...
      final beds = snapshot.docs.map((doc) {
        ...
        if (rawDate is Timestamp) {
          ...
          createdAt =
              DateTime.tryParse(rawDate) ?? createdAt;
        }
  ...
  void _reloadBeds() {
    _bedsFuture = _loadBedsFromFirebase();
  }
