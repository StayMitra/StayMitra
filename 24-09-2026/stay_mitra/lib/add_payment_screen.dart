...
class _AddPaymentScreenState extends State<AddPaymentScreen> {
  ...
  Future<void> _loadRooms() async {
    ...
    try {
      ...
      final snapshot = await _propertyRef(activePg.id)
          .collection('rooms')
          .get();
      ...
      rooms.sort((a, b) {
        ...
        final aInt = int.tryParse(aNumber);
        final bInt = int.tryParse(bNumber);
      ...
      _showError('Unable to load rooms: $e');
    }
  }

  // ============================================================
  // LOAD ACTIVE TENANTS FOR SELECTED ROOM - FIREBASE ONLY
  // ============================================================
  ...
  Future<void> _loadTenantsForRoom(
    ...
    try {
      ...
      if (activePg == null) {
        throw Exception('No active PG selected.');
      }

      final propertyRef = _propertyRef(activePg.id);

      // Get beds belonging to the selected room.
      final bedsSnapshot = await propertyRef
          .collection('beds')
          .where('room_id', isEqualTo: roomId)
          .get();

      final bedIds = bedsSnapshot.docs
          .map((doc) => doc.id)
          .toSet();
      ...
      // Read tenants from Firebase and keep only active tenants
      // whose bed belongs to the selected room.
      final tenantsSnapshot =
          await propertyRef.collection('tenants').get();
      ...
      for (final doc in tenantsSnapshot.docs) {
        ...
        final bedId =
            (data['bed_id'] ?? data['bedId'] ?? '')
                .toString();

        if (status != 'active' ||
            !bedIds.contains(bedId)) {
          continue;
        }

        final bedDoc =
            bedsSnapshot.docs.where(
          (bed) => bed.id == bedId,
        );

        String bedNumber = '';

        if (bedDoc.isNotEmpty) {
          final bedData = bedDoc.first.data();
        ...
        tenants.add({
          ...
          'phone':
              (data['phone'] ?? '').toString(),
          'monthly_rent':
              (data['monthly_rent'] as num?)?.toDouble() ?? 0,
          'bed_id': bedId,
      ...
      _showError('Unable to load tenants: $e');
    }
  }
  ...
  Future<void> _savePayment() async {
    ...
    try {
      ...
      await _propertyRef(activePg.id)
          .collection('transactions')
          .doc(transaction.id)
          .set({
        'id': transaction.id,
        'building_id': transaction.buildingId,
  ...
  Widget _tenantSelector() {
    ...
    return DropdownButtonFormField<String>(
      ...
      items: _roomTenants.map((tenant) {
        final tenantId =
            tenant['tenant_id'] as String;
        final tenantName =
            tenant['tenant_name'] as String;
        final bedNumber =
            tenant['bed_number'] as String;
