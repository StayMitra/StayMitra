...
class _RoomsScreenState extends State<RoomsScreen> {
  ...
  // 👇 EXISTING _loadRooms() METHOD NI IKKADA REPLACE CHEYYALI
...
Future<void> _loadRooms() async {
  ...
  try {
    ...
    debugPrint('========================================');
    debugPrint('ROOMS SCREEN - ACTIVE PG');
    debugPrint('PG NAME : ${activePg.name}');
    debugPrint('PG ID   : ${activePg.id}');
    debugPrint('========================================');
    ...
    // ==========================================================
    // GET FLOORS FOR ACTIVE PG ONLY - FIREBASE
    // ==========================================================

    final floors = await FirebaseTenantService.instance.getFloors(
      propertyId: activePg.id,
    );

    // Load tenants once for the active PG.
    // This replaces the old local getActiveTenantByBed() calls.
    final tenants = await FirebaseTenantService.instance.getTenants(
      propertyId: activePg.id,
    );

    final activeTenantsByBedId = <String, TenantModel>{};

    for (final tenant in tenants) {
      if (tenant.status.trim().toLowerCase() == 'active' &&
          tenant.bedId.trim().isNotEmpty) {
        activeTenantsByBedId[tenant.bedId] = tenant;
      }
    }
    ...
    // ==========================================================
    // GET ROOMS + BEDS FOR ACTIVE PG - FIREBASE
    // ==========================================================

    for (final floor in floors) {
      final rooms = await FirebaseTenantService.instance.getRooms(
        propertyId: activePg.id,
        floorId: floor.id,
      );
      ...
      for (final room in rooms) {
        ...
        final beds = bedsSnapshot.docs.map((doc) {
          final data = doc.data();

          return BedModel(
            id: doc.id,
            roomId: (data['room_id'] ?? data['roomId'] ?? room.id).toString(),
            bedNumber:
                (data['bed_number'] ?? data['bedNumber'] ?? '').toString(),
        ...
        for (final bed in beds) {
          final tenant = bed.status.trim().toLowerCase() == 'occupied'
              ? activeTenantsByBedId[bed.id]
              : null;
    ...
    debugPrint(
      'Loaded ${entries.length} rooms for PG: ${activePg.name}',
    );
  } catch (e) {
    if (!mounted) return;
