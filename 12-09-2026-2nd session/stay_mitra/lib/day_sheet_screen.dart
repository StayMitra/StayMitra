...
class _DaySheetScreenState extends State<DaySheetScreen> {
  ...
  // The Day Sheet must always belong to the currently selected PG.
  ...
  Future<void> _loadTransactions() async {
    ...
    // IMPORTANT: Never show transactions from another PG.
    // PgManagerService contains the PG selected on the Dashboard.
  ...
  // Always read the CURRENT PG from PgManagerService.
  // This is important when the owner switches PG and comes back to
  // Day Sheet without recreating this screen.
  BuildingModel? get _currentPg =>
      PgManagerService.activePg ?? _activePg;

  List<TransactionModel> get _allTransactions {
    final buildingId = _currentPg?.id?.trim();

    // No active PG = no financial data. This is safer than accidentally
    // displaying another PG's transactions.
    if (buildingId == null || buildingId.isEmpty) {
      return const <TransactionModel>[];
    }

    // IMPORTANT: Day Sheet is strictly PG-specific.
    // A transaction is included only when its buildingId matches
    // the currently selected PG.
    return TransactionService
        .getTransactions()
        .where((transaction) =>
            transaction.buildingId?.trim() == buildingId)
        .toList();
  }
